/*==============================================================*\
 *  cliqhelp.c - routines for the help manager interface
...
VOID InitHelp(VOID)
{
   ...
   hini.phtHelpTable = (PHELPTABLE)MAKELONG(CLIQ_HELP_TABLE, 0xFFFF);
   hini.hmodHelpTableModule = 0L;
   hini.hmodAccelActionBarModule = 0L;
   hini.idAccelTable = 0;
   hini.idActionBar = 0;

   if(!(apiRC=WinLoadString(hab,
                     0,
                     IDS_HELPWINDOWTITLE,
                     HELPLIBRARYNAMELEN,
                     (PSZ)szWindowTitle)))
   {
      MessageBox(hwndMain, IDMSG_CANNOTLOADSTRING, apiRC, MB_OK | MB_ERROR, FALSE);
      return;
   }
   ...
   {
      MessageBox(hwndMainFrame, IDMSG_HELPLOADERROR, hini.ulReturnCode, MB_OK | MB_ERROR, TRUE);
      return;
   }

   /* associate help instance with main frame */
   if(!(apiRC=WinAssociateHelpInstance(hwndHelpInstance, hwndMainFrame)))
   {
      MessageBox(hwndMainFrame, IDMSG_HELPLOADERROR, apiRC, MB_OK | MB_ERROR, TRUE);
      return;
   }
...
VOID  HelpExtended(MPARAM mp2)
{
   ...
   if(fHelpEnabled)
   {
      if((BOOL32)WinSendMsg(hwndHelpInstance, HM_EXT_HELP, MPVOID, MPVOID))
      {
         MessageBox(hwndMain,
                    IDMSG_HELPDISPLAYERROR,
                    0,
