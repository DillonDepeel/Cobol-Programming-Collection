/*static char *SCCSID = "@(#)sem_help.c	6.17 92/02/19";*/
/*==============================================================*\
 *  SEM_HELP.C - routines for help information                  *
...
VOID InitHelp(VOID)
{
   ...
   hini.phtHelpTable = (PHELPTABLE)MAKELONG(SEMAPH_HELP_TABLE, 0xFFFF);
   ...
   if(!WinLoadString(hab,
                     ...
                     (PSZ)szWindowTitle))
   {
       MessageBox(hwndMain, IDMSG_CANNOTLOADSTRING,TRUE);
       return;
   }
...
VOID HelpHelpForHelp(MPARAM mp2)
{
   ...
   if(fHelpEnabled)
     if((LONG)WinSendMsg(hwndHelpInstance,HM_DISPLAY_HELP,
                                 (MPARAM)0,(MPARAM)0))
           MessageBox(hwndMain, IDMSG_HELPDISPLAYERROR,TRUE);
...
VOID DisplayHelpPanel(LONG idPanel)
{
   if(fHelpEnabled)
       if((LONG)WinSendMsg(hwndHelpInstance,
           ...
           MessageBox(hwndMainFrame, IDMSG_HELPDISPLAYERROR, TRUE);
   return;
}                                         /* DisplayHelpPanel() */
