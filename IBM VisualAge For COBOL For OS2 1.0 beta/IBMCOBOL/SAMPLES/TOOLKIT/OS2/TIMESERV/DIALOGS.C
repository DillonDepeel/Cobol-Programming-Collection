...
MRESULT EXPENTRY AlarmDlgProc(HWND hwnd , ULONG usItem,
                               MPARAM mp1, MPARAM mp2 );
MRESULT EXPENTRY EntryFieldSubProc(HWND hwnd, ULONG usItem,
                                     MPARAM mp1, MPARAM mp2 );
/**************************************************************************\
*                                                                          *
...
MRESULT EXPENTRY ClkTicksDlgProc ( HWND hwnd , ULONG usMsg ,
                                   MPARAM mp1 , MPARAM mp2 )
{
    ...
    switch ( usMsg ) {
        ...
        case WM_INITDLG :
            ...
            WinSendMsg ( WinWindowFromID ( hwnd ,
                         ...
                         BM_SETCHECK , MPFROM2SHORT ( TRUE , NULL ) , MPVOID) ;
...
MRESULT EXPENTRY AlarmDlgProc ( HWND hwnd , ULONG usMsg ,
                                   MPARAM mp1 , MPARAM mp2 )
{
    ...
    switch ( usMsg ) {
        ...
        case WM_INITDLG :
            hAlarmTime = hwnd; /*put in a global var, so everybody know we already
                                have an opened dialog box*/
            InitAlarm(hwnd);
...
void InitAlarm(HWND hwnd)
{
    ...
    WinSendMsg( WinWindowFromID(hwnd,SCROLL_HOURS),SBM_SETSCROLLBAR,
                MPFROMLONG(2),
                MPFROM2SHORT(1,3));
    WinSendMsg( WinWindowFromID(hwnd,SCROLL_MINUTES),SBM_SETSCROLLBAR,
                MPFROMLONG(2),
                MPFROM2SHORT(1,3));
    ...
    WinSendDlgItemMsg ( hwnd ,IDC_HOURMINUTES, EM_SETSEL , MPFROM2SHORT(0,0),
                        MPVOID  ) ;
...
VOID AlarmSetTimer(UCHAR uchHour,UCHAR uchMinutes )
{
     ...
     */
     ...
     TimerResources.ulTimerValue  = ( UTCAlarmTime  - UTCCurrentTime)
                                          * 1000L ;
     /*
      *post the timer
      */
...
*                                                                          *
\**************************************************************************/
MRESULT EXPENTRY ClkColorsDlgProc ( HWND hwnd , ULONG usMsg ,
                                   MPARAM mp1 , MPARAM mp2 )
{
    USHORT usButtonID;
    ...
    switch (usMsg)
    {
        ...
        case WM_INITDLG :
            ...
            GpiCreateLogColorTable(hpsT, LCOL_RESET, LCOLF_RGB, 0L, 0L,(PLONG)NULL);
            ...
            WinPostMsg ( WinWindowFromID ( hwnd , CLKCLR_BACKGROUND ) ,
        ...
        case WM_COMMAND :
            ...
            switch ( LOUSHORT ( mp1 ) ) {
                case DID_OK:
                    ...
                    GpiCreateLogColorTable( hpsT,LCOLF_RGB,
                                            0L, 0L, 0L,NULL );

                    /*
                     *update the one the user has
                     *selected
                     */
                    switch( usCheckedPartID )
                    {
                    case CLKCLR_BACKGROUND:
                         cp.clrBackground = GpiQueryRGBColor(hpsT, 0L,
                                                clrBackgroundNew);
                         break;
                    case CLKCLR_FACE:
                         cp.clrFace = GpiQueryRGBColor(hpsT, 0L,
                                             clrFaceNew);
                         break;
                    case CLKCLR_HOURHAND:
                         cp.clrHourHand = GpiQueryRGBColor(hpsT, 0L,
                                             clrHourHandNew);
                         break;
                    case CLKCLR_MINUTEHAND:
                         cp.clrMinuteHand = GpiQueryRGBColor(hpsT, 0L,
                                             clrMinuteHandNew);
                         break;
                    }
                    WinReleasePS(hpsT);
        ...
        case WM_CONTROL :
            ...
            if ( usButtonID & CLKCLR_OBJECTS ) {
                ...
                rc=WinSendMsg (WinWindowFromID(hwnd, (CLKCLR_BUTTONSHIFT + *pclrNew)),
                             BM_CLICK , MPFROMSHORT ( TRUE ) , MPVOID) ;
                break ;
            }
...
* current selection. It doesn't allow modification of the time seperator.  *
...
MRESULT EXPENTRY EntryFieldSubProc ( HWND hwnd , ULONG usMsg ,
                                   MPARAM mp1 , MPARAM mp2 )
{
    ...
    if ((usMsg > WM_MOUSEFIRST) && (usMsg < WM_MOUSELAST)) {
        mresult = pfnwpEntryField ( hwnd , usMsg , mp1 , mp2 ) ;

        mresultSel = pfnwpEntryField(hwnd,EM_QUERYSEL,(PLONG)NULL,(PLONG) NULL);
        usSelect = LOUSHORT(mresultSel);
        usLast = HIUSHORT(mresultSel);
        if ((usSelect == 2) )
               pfnwpEntryField ( hwnd , EM_SETSEL , MPFROM2SHORT(3,3) ,
                                (PVOID)NULL ) ;
        if ((usSelect == 5) )
               pfnwpEntryField ( hwnd , EM_SETSEL , MPFROM2SHORT(4,4) ,
                                    (PVOID)NULL) ;
        return(mresult);
    }
            ...
            if (fsKeyFlags & KC_CHAR )
            {
                ...
                case 0:   /*Hours most significant digit*/
                    ...
                    if (bValid)
                    {
                        ...
                        pfnwpEntryField ( hwnd , EM_SETSEL , MPFROM2SHORT(1,1) ,
                                            (PLONG)NULL ) ;
                    }
                    return(MPVOID);
                    break;
                case 1:
                    ...
                    /*Full the entryfield as if a time seperator is in also*/
                    if (bValid) {
