...
//BIDI - Create the MLE so that it occupies only part of the client window
//       This enables us to use the high part of the window for
//       other output.

    hwndMLE = WinCreateWindow(hwnd, WC_MLE, (PSZ)NULL,
                              MLS_HSCROLL | MLS_VSCROLL | WS_VISIBLE,
                              (SHORT)rcl.xLeft, (SHORT)rcl.yBottom,
                     // BIDI  (SHORT)rcl.xRight, (SHORT)rcl.yTop,
                              (SHORT)rcl.xRight, (SHORT)rcl.yTop/2,
