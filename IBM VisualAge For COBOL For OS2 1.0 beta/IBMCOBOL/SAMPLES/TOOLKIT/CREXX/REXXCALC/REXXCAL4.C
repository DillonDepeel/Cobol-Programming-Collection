/*********************************************************************/
/*                                                                   */
/* REXXCAL4 - A simple PM based pocket calculator                    */
/*                                                                   */
/*********************************************************************/
...
/* Global data used for the calculator                               */
 ...
 LONG     editted_program;             /* macro editted              */
 ...
 ULONG    global_completion;           /* completion message         */
 ULONG    global_macro_name;           /* macro to invoke            */
                 ...
                 switch (memory_action) {
                   ...
                   case BUTTON_MEMORY_CLEAR:
                     ...
                     ProcessMacro(memory_action - MIN_MACRO,
                         &memory_setting, NULL, NULL, WM_MEMORY_DONE);
                     break;            /* finished                   */
  ...
  PINSTORE   descriptor )              /* saved location             */
{
  ...
  MAKERXSTRING(descriptor->image, NULL, 0);
                                       /* open the file              */ 
  DosOpen(program, &handle, &action, 0, 0, OPEN_ACTION_OPEN_IF_EXISTS,
...
{
  strcpy(RXSTRPTR(*retstr), precision);/* copy over current precision*/
                                       /* set new length             */
  retstr->strlength = strlen(precision);
...
void EditThread(void)
{
  ...
  argptr = args + sizeof("E");         /* step past name             */
                                       /* copy in file name          */
  strcpy(argptr, operator_names[editted_program]);
                                       /* terminate arguments        */
  argptr[strlen(operator_names[editted_program]) + 1] = 0;
...
/*********************************************************************/
/*                                                                   */
/* CalcThread    - Execute a macro bound to a key                    */
/*                                                                   */
/*********************************************************************/
