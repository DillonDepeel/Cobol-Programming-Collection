...
PHEAP NewHeap (PHEAP pFirstHeap, ULONG lInitSize, USHORT fFlags)
{
   ...
   pHeap = (PHEAP) HhpAllocBuffer (lInitSize, fFlags);
...
//   memset( pHeap, '\0', (ULONG)lInitSize );
   ...
   /* Init Block Size */
   pHeap->pBlock->lBlockSize = (lInitSize - (ULONG) sizeof(HEAP));
...
PVOID APIENTRY HhpAllocMem (HHUGEHEAP hheap, ULONG cbLen)
{
   ...
   /* Locate a free Block */
   fAlloc = LocateFreeMem (pFirstHeap, &pHeap, &pBlock, cbLen);
                  ...
                  ULONG lSize)
{
   ...
   for (pHeap = pFirstHeap; pHeap; pHeap = pHeap->pNext)
   {
      ...
      if ((LONG)0xffffL - (LONG)pHeap->lSize >= (LONG)lSize)
      { /* ReSize segment */
         ...
         /* Last */
         for (pBlock = pHeap->pBlock; pBlock->pNext; pBlock = pBlock->pNext);
         if (pBlock->fAlloc)                       /* Last Block Allocated */
         {
            pNextBlock = (PBLOCK)((PCHAR)pBlock + sizeof(BLOCK) +
                                   pBlock->lBlockSize);
