...
BOOL  QueryDirectMode()
{
    UCHAR        LoadError[256];       /* DosLoadModule Error     */
    HMODULE      ModuleHandle;         /* DIVE.DLL handle         */
    DIVE_CAPS    DiveCaps;             /* Dive Query Capabilities */
    ULONG        rc;                   /* API Return Code         */
    FOURCC       fccFormats[100] = {0};
    ULONG (* APIENTRY  pfnDiveCaps) (PDIVE_CAPS,ULONG);

    rc = DosLoadModule((PSZ)LoadError, sizeof(LoadError),(PSZ)"DIVE",
                       &ModuleHandle);
    if (rc)
       return (TRUE);      // We Still may be able to run.

    rc = DosQueryProcAddr(ModuleHandle, 0L,(PSZ)"DiveQueryCaps",
                         (PFN *) &(pfnDiveCaps));
    if (rc)
       return (TRUE);
    ...
    rc = pfnDiveCaps (&DiveCaps,DIVE_BUFFER_SCREEN);
    if (!rc)
      {
