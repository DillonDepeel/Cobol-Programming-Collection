...
switch( msg )
    {
         ...
         case WM_COMMAND:
              ...
              switch (command)
              {
                   ...
                   case ID_FILE_OPEN:
                        ...
                        pszSource = malloc ( MAX_FILENAME_SIZE );
                        ...
                        if( OpenFileDialog ( hwnd,   // handle of owner window
                                             ...
                                             "*.*",                    // file extension
                                             szOpenOKButton,
                                             pszSource ) == FALSE )  // return filename here.
                        {
                             break;
                             // error.
                        }

                        bReturnCode = (BOOL) GetBitMap ( hwnd, pszSource );

                        WinInvalidateRegion( hwnd, 0, FALSE );
                   break;
                   case ID_FILE_SAVEAS:
                        if( mmioIdentifyFile(pszSource,
                            0L,
                            &mmFormatInfoSource,
                            &IOProcFourSource,
                            0L,
                            0L))
     {
                        ...
                        SaveFile( hwnd, pszSource, fccSourceIOProc);
                        WinInvalidateRegion( hwnd, 0, FALSE );
                        break;
